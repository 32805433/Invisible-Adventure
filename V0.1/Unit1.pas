...
var
  ...
  minattack,maxattack,monsterlife,minmonsterattack,maxmonsterattack:integer;
  ...
  find,fish,start,lh,fight,pon,pa,pb,pc:boolean;
...
begin
  ...
  if p=1 then
    begin
      ...
      case a[x,y] of
        ...
        16:begin
             groupbox5.Visible:=true;Button13.SetFocus;
        ...
        22:begin
             ...
             if pa=true then Button10.Visible:=false;
             if pb=true then Button15.Visible:=false;
             Button5.Visible:=true;
             if gold>=10 then Button10.Enabled:=true;
             if gold>=12 then Button15.Enabled:=true;
             if gold>=3 then Button16.Enabled:=true;
             if gold<10 then Button10.Enabled:=false;
             if gold<12 then Button15.Enabled:=false;
             if gold<3 then Button16.Enabled:=false;
  ...
  if p=4 then
    begin
      Label2.Caption:='僧人说："施主这么有缘，教你一种武功吧！"'+chr(13)+chr(10)+'你学会了罗汉拳！普通攻击伤害升至3~4...'+chr(13)+chr(10)+'僧人消失了...';
      lh:=true;change:=2;
...
begin
  ...
  b[4]:=true;b[5]:=true;b[7]:=true;b[9]:=true;b[10]:=true;b[14]:=true;b[15]:=true;b[17]:=true;b[19]:=true;b[20]:=true;b[21]:=true;b[23]:=true;b[24]:=true;
  fillchar(cansay,sizeof(cansay),true);
  s:=extractfilepath(application.ExeName);
  wall:=Tbitmap.Create;
  wall.LoadFromFile(s+'icon/(c)wall1.bmp');
  start:=false;
end;
 ...
 if opendialog1.Execute() then
  begin
   try
    ...
    pon:=false;fight:=false;lh:=false;life:=2;maxhitpoints:=60;hitpoints:=60;ropes:=0;keys:=0;gold:=10;torch:=0;bulb:=0;x:=1;y:=1;win:=0;
    closefile(f);
   except
    showmessage('读取失败!');
    exit;
 ...
 if opendialog1.Execute() then
  begin
   try
    ...
    readln(f,life,hitpoints,maxhitpoints,ropes,keys,gold,torch,bulb,lt,win);readln(f,x,y);
...
begin
 try
  ...
  writeln(f,life,' ',hitpoints,' ',maxhitpoints,' ',ropes,' ',keys,' ',gold,' ',torch,' ',bulb,' ',lt,' ',win);
  writeln(f,x,' ',y);
  for i:=1 to size do
   for j:=1 to size do
    if hints[i,j]<>'' then
...
begin
 if start then
  begin
   if messagebox(form1.Handle,'确定要重新开始吗?','询问',mb_yesno+mb_iconquestion)=7 then exit;
   Button1.Enabled:=false;Button2.Enabled:=false;Button3.Enabled:=false;Button4.Enabled:=false;Button5.Visible:=false;
   N9.Caption:='开始游戏';N9.Enabled:=false;
   Button7.Enabled:=false;Button8.Enabled:=false;Button7.Visible:=false;Button8.Visible:=false;
   N3.Enabled:=true;N4.Enabled:=true;groupbox2.Visible:=false;groupbox3.Visible:=false;
...
begin
  ...
  if winem=0 then N05.Visible:=false;
  if curem=0 then N06.Visible:=false;
  if minem=0 then N07.Visible:=false;
  if lightm=0 then N02.Visible:=false;
